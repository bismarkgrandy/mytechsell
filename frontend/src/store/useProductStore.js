...
export const useProductStore = create((set) => ({
  ...
  fetchProducts: async () => {
    try {
      const response = await axiosInstance.get(
        "http://localhost:3000/api/products?limit=15&page=1"
      );
  ...
  fetchFeaturedProducts: async () => {
    try {
      const response = await axiosInstance.get("http://localhost:3000/api/products/featured");
  ...
  searchProducts: async (keyword) => {
    ...
    set({ loading: true });
    try {
      const response = await axiosInstance.get(`http://localhost:3000/api/products/search?keyword=${keyword}`);
