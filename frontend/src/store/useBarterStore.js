...
export const useBarterStore = create((set) => ({
  ...
  fetchBarterItems: async () => {
    try {
      const response = await axiosInstance.get("http://localhost:3000/api/barter");
  ...
  listItem: async (formData) => {
    ...
    try {
      const response = await axiosInstance.post("http://localhost:3000/api/barter/list-item", formData);
      set((state) => ({
  ...
  delistItem: async (itemId, authUserId, ownerId) => {
    ...
    try {
      await axiosInstance.patch(`http://localhost:3000/api/barter/delist/${itemId}`);
      set((state) => ({
